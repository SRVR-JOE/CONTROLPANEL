    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px;">
        <div style="background: ${severityColors[event.severity] || '#6b7280'}; color: white; padding: 12px 16px; border-radius: 8px 8px 0 0;">
          <strong>${event.severity.toUpperCase()}</strong> — ${event.eventType.replace('_', ' ')}
        </div>
        <div style="border: 1px solid #e5e7eb; border-top: none; padding: 16px; border-radius: 0 0 8px 8px;">
          <h2 style="margin: 0 0 8px 0;">${event.title}</h2>
          <p style="color: #6b7280; margin: 0 0 12px 0;">${event.message}</p>
          <table style="font-size: 14px; color: #374151;">
            <tr><td style="padding: 2px 12px 2px 0; color: #9ca3af;">Device</td><td>${event.deviceName}</td></tr>
